...
function like_button($atts): string {
    ...
    $output .= '<input type="hidden" id="post_id" name="post_id" value="' . $post_id . '">';
...
#[NoReturn] function add_like(): void
{
    ...
    $preparedQuery = $wpdb->prepare( "SELECT user_id FROM $table_name WHERE post_id = %d AND user_id = %d", $data );
...
function like_button_enqueue_scripts(): void {
    wp_register_script( 'like-button', plugin_dir_url(__FILE__) . '/likeButton.js', [], '1.0', true );
    $script_data = array(
